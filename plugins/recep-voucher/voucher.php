 ...
 * License: MIT
 */
 function VoucherReceptivo($attributes)
 {
...
<input type="hidden" name="URLretorno" id="URLretorno" value="<?php='https://www.viajesvivatours.com/voucher-receptivo/'?>">
  ...
  <table width="100%" border="1" align="center" cellpadding="0" cellspacing="0" rules="none" class="table table-bordered table-hover table-condensed">
    ...
    <tr>
      ...
      <td width="26%" height="110" class="logo_voucher" valign="middle" align="center"><img src="https://viajesvivatours.com/wp-content/plugins/recep-voucher/images/Voucher_Logo.jpg" width="180" height="80" /></td>
...
<label>Criterio de filtrado:(Usar cliente, confirmado o prestatario) por: </label><div class="input-group">
 ...
 <input name="Submit2" type="button" onclick="location.href='?'" value="ACTUALIZAR LISTADO" class="btn btn-warning"/>  <input name="Button" type="button" class="btn btn-success" value="CREAR NUEVO" onclick="location.href='https://viajesvivatours.com/voucher-receptivo/?nuevo=1'"/></div>
    </form>
<hr />
...
<thead/>
 ...
 <th width="3%" height="29" align="center" style="text-align:center !important;">/</th>
   ...
   <th width="7%" height="29"   class="">FOLIO</th>

<th width="6%" class="">CODE</th>
<th width="5%" class="">FECHA</th>
<th width="19%" class="">CLIENTE</th>
<th width="19%" class="">CONFIRMADO</th>
<th width="19%" class="" >PRESTATARIO</th>
    <th width="22%" class="" >RESERVADO</th>
  </tr>
</thead>  <tbody>
  <?php
...
$buscar=$_GET['buscar'];
  ...
  if ($numero==1) {
   ...
   $_pagi_sql="SELECT * FROM vvt_voucher_recep WHERE (codigo3 LIKE '".$buscar."%' OR codigo9 LIKE '".$buscar."%' OR codigo10 LIKE '".$buscar."%')";
  } elseif ($numero>1) {
  //SI HAY UNA FRASE SE UTILIZA EL ALGORTIMO DE BUSQUEDA AVANZADO DE MATCH AGAINST
  //busqueda de frases con mas de una palabra y un algoritmo especializado
  $_pagi_sql="SELECT *, MATCH (codigo3,codigo9,codigo10) AGAINST ( '".$buscar."' ) AS Score FROM vvt_voucher_recep WHERE MATCH (codigo3,codigo9,codigo10) AGAINST ( '".$buscar."' ) ";
  } 
//////
	 }
else{
	//	if($ordenar)
$_pagi_sql = "SELECT * FROM vvt_voucher_recep  ORDER BY idvoucher DESC"; 
 }
$orden=$_GET[orden];
...
$datos = mysql_query("SELECT * FROM vvt_voucher_recep");
...
<td style='text-align:center !important;'><a href='../ver-voucher-receptivo/?folio=".$row['idvoucher']."&URLretorno=".$URLretorno."' title='Consultar Voucher' ></a>
";
...
echo "</td>
<td><a href='../ver-voucher-receptivo/?folio=".$row['idvoucher']."&URLretorno=".$URLretorno."' title='Consultar Voucher' >".sprintf("%08d",$row['idvoucher'])."</a></td>
<td><a href='../ver-voucher-receptivo/?folio=".$row['idvoucher']."&URLretorno=".$URLretorno."' title='Consultar Voucher' >".$row['codigo1']."</a></td>
<td><a href='../ver-voucher-receptivo/?folio=".$row['idvoucher']."&URLretorno=".$URLretorno."' title='Consultar Voucher' >".$row['codigo2']."</a></td>
<td><a href='../ver-voucher-receptivo/?folio=".$row['idvoucher']."&URLretorno=".$URLretorno."' title='Consultar Voucher' >".$row['codigo9']."</a></td>
<td><a href='../ver-voucher-receptivo/?folio=".$row['idvoucher']."&URLretorno=".$URLretorno."' title='Consultar Voucher' >".$row['codigo3']."</a></td>
<td><a href='../ver-voucher-receptivo/?folio=".$row['idvoucher']."&URLretorno=".$URLretorno."' title='Consultar Voucher' >".$row['codigo10']."</a></td>
<td><a href='../ver-voucher-receptivo/?folio=".$row['idvoucher']."&URLretorno=".$URLretorno."' title='Consultar Voucher' >".$row['codigo13']."</a></td>
     </tr>"; 
...
?></tfoot></table>
<?php }
 add_shortcode('VoucherReceptivo', 'VoucherReceptivo');