jQuery(document).ready(function() {
  var Closed = false;
  jQuery('.ewd-feup-pointer-exit').on('click', function() {
    Closed = true;
  })
});
...
function FEUP_Dismiss_Pointers() {
  jQuery('.feup-skip-all-tutorial-pop-ups').on('click', function() {
    jQuery.post( ajaxurl, { action: 'feup-dismiss-wp-pointers'});
...
jQuery(document).ready(function($) {
  jQuery('body').on('DOMNodeInserted', function(event) {
    if (jQuery(event.target).hasClass('ewd-feup-pointer-count')) {FEUP_Dismiss_Pointers();}
  });
});