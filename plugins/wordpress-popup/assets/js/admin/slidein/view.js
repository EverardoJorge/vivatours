Hustle.define("Slidein.View", function($, doc, win){
	...
	return Hustle.View.extend({
		...
		provider_add_custom_fields: function(id, nonce, changed, me) {
			...
			if ( Module.Utils.service_supports_fields( 0, active_email_service ) !== true ||  (
			...
			if ( typeof me.content_view.model.get('form_elements') === 'object' ) {
				...
				$.post( ajaxurl, data, function( response ){}).fail(function( response ) {
		...
		render_preview: function(content_model) {
			...
			var data = _.extend( me.model.toJSON(), {
		...
		apply_preview_styles: function() {
			...
			var $preview_modal        = this.$('#wph-preview-modal'),
				...
				$content              = $modal.find('.hustle-modal-message, .hustle-gdpr-content'),
